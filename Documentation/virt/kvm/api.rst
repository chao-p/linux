...
The user should check the return value as well as the size to decide if the
operation succeeded for the whole range or not. The user may want to retry the
operation with the returned struct if the previous range partially succeeded.
