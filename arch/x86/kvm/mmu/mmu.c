			      ...
			      int max_level)
{
	...
	return __kvm_mmu_max_mapping_level(kvm, slot, gfn, max_level, PG_LEVEL_NONE, is_private);
}
				   ...
				   struct kvm_page_fault *fault)
{
	...
	r = kvm_restricted_mem_get_pfn(fault->slot, fault->gfn, &fault->pfn, &order);
	if (r)
		return r;
		      ...
		      bool map_private)
{
	...
	attr = map_private ? KVM_MEMORY_ATTRIBUTE_PRIVATE : 0;
